(function ($) {
    ...
    $.learunindex = {
        load: function () {
            ...
            $("#content-wrapper").find('.mainContent').height($(window).height() - 78);
            ...
            $(".sidebar-toggle").click(function () {
                if (!$("body").hasClass("sidebar-collapse")) {
                    ...
                    localStorage.setItem("isShow","close");
                } else {
                    $("body").removeClass("sidebar-collapse");
                    localStorage.setItem("isShow","open");
                }
            })
                    ...
                    if (childNodes.length > 0) {
                        ...
                        $.each(childNodes, function (i) {
                            ...
                            if (subchildNodes.length > 0) {
                                _html += '<a href="#"><i class=" ' + (subrow.icon=="NULL"?"":subrow.icon )+ '"></i>&nbsp;&nbsp;' + subrow.name + '';
                                _html += '<i class="fa fa-angle-left pull-right"></i></a>';
                                _html += '<ul class="treeview-menu">';
                                $.each(subchildNodes, function (i) {
                                    var subchildNodesrow = subchildNodes[i];
                                    $("#menus").append("<option value='"+subchildNodesrow.name+"'>"+subrow.name+"</option>");
                                    menuArr.push({id:subchildNodesrow.id,text:subchildNodesrow.name});
                                    _html += '<li><a id="menu_'+subchildNodesrow.id+'" class="menuItem" data-id="' + subrow.id + '" href="' + subchildNodesrow.runConfig + '"><i class="' + (subchildNodesrow.icon=="NULL"?"":subchildNodesrow.icon ) + '"></i>' + subchildNodesrow.name + '</a></li>';
                                });

                                _html += '</ul>';

                            } else {
                                $("#menus").append("<option value='"+subrow.name+"'>"+subrow.name+"</option>");
                                menuArr.push({id:subrow.id,text:subrow.name});
                                _html += '<a id="menu_'+subrow.id+'"  class="menuItem" data-id="' + subrow.id + '" href="' + subrow.runConfig + '"><i class="' +  (subrow.icon=="NULL"?"":subrow.icon ) + '"></i>' + subrow.name + '</a>';
                            }
                            _html += '</li>';
                        });
                        _html += '</ul>';
            ...
            $("#sidebar-menu li a").click(function () {
                ...
                if (e.is(".treeview-menu") && e.is(":visible")) {
                    ...
                    e.slideDown(500, function () {
                        ...
                        var _height1 = $(window).height() - $("#sidebar-menu >li.active").position().top - 31;
